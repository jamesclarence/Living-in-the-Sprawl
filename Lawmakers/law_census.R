...
demc1 <- data.frame(xmin_d1 = factor(93), xmax_d1 = factor(103), ymin_d1 = -Inf, ymax_d1 = Inf)
demc2 <- data.frame(xmin_d2 = factor(110), xmax_d2 = factor(111), ymin_d2 = -Inf, ymax_d2 = Inf)
...
rect_cd1 <- geom_rect(data = demc1, aes(xmin = xmin_d1, xmax = xmax_d1, ymin = ymin_d1, ymax = ymax_d1), 
                     fill = "blue", alpha=0.25, inherit.aes = FALSE)
rect_cd1 <- geom_rect(data = demc2, aes(xmin = xmin_d2, xmax = xmax_d2, ymin = ymin_d2, ymax = ymax_d2), 
                     fill = "blue", alpha=0.25, inherit.aes = FALSE)
...
ggplot(grp, aes(x = congress, y = les, shape = majority, color = majority)) +
...
# Plot 1: Number of members per Region per Congress
grp_2 %>%
    summarise(members=n()) %>%
...
# Plot 2: LES score per region per region's member
grp_2 %>%
...
# Plot 3: In depth look at South's sub-division